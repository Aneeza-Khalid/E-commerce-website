...
export default function ShopDetailEight() {
  ...
  const handleAddToCart = () => {
    ...
    const existingItemIndex = cartItems.findIndex(item => item.name === 'Ultra Portable 4K');
    if (existingItemIndex !== -1) {
      // If the product is already in the cart, update its quantity
      const updatedCartItems = [...cartItems];
      updatedCartItems[existingItemIndex].quantity += parseInt(quantity); // Parse quantity to ensure it's a number
      setCartItems(updatedCartItems);
    } else {
      // If the product is not in the cart, add it as a new item
      const newItem = {
        id: cartItems.length + 1,
        name: 'Ultra Portable 4K', // Unique product name
        price: 699.0,
        quantity: parseInt(quantity, 10), // Parse quantity to ensure it's a number
        imageUrl: "https://i.ibb.co/SvPxs25/8th.jpg",
      };
                ...
                >
                  <Carousel.Item className='w-100'>
                    <img
                      ...
                      src="https://i.ibb.co/SvPxs25/8th.jpg"
                      alt="First slide"
                      width="800"
                      height="400"
                    />
